...
TP_BIN="${TP_BIN:-./target/debug/tplane}"
WORKSPACE="${THREADPLANE_DEMO_WORKSPACE:-demo-lab}"
DEMO_COMMAND_PAUSE_SECONDS="${DEMO_COMMAND_PAUSE_SECONDS:-2.8}"
DEMO_RESULT_PAUSE_SECONDS="${DEMO_RESULT_PAUSE_SECONDS:-3.8}"
DEMO_INTRO_PAUSE_SECONDS="${DEMO_INTRO_PAUSE_SECONDS:-1.8}"
...
run 'tplane scope 2>/dev/null | jq "{name, log: .poc.authoritative_log, graph: .poc.graph_projection}"' \
    "\"$TP_BIN\" scope 2>/dev/null | jq '{name, log: .poc.authoritative_log, graph: .poc.graph_projection}'"
run "tplane epic list --workspace \"$WORKSPACE\" | jq '.data[] | {title}'" \
    "\"$TP_BIN\" epic list --workspace \"$WORKSPACE\" | jq '.data[] | {title}'"
run "tplane task list --workspace \"$WORKSPACE\" --status open --format compact" \
    "\"$TP_BIN\" task list --workspace \"$WORKSPACE\" --status open --format compact"
run "tplane task blocked-by --task-id \"$task_b_id\"" \
    "\"$TP_BIN\" task blocked-by --task-id \"$task_b_id\""
run "tplane task blocks --task-id \"$task_a_id\" --direct-only" \
    "\"$TP_BIN\" task blocks --task-id \"$task_a_id\" --direct-only"
run "tplane task complete --workspace \"$WORKSPACE\" --actor operator --task-id \"$task_a_id\" | jq '{task_id: .data.task_id, status: .data.status}'" \
    "\"$TP_BIN\" task complete --workspace \"$WORKSPACE\" --actor operator --task-id \"$task_a_id\" | jq '{task_id: .data.task_id, status: .data.status}'"
run "tplane task list --workspace \"$WORKSPACE\" --status open --ready-only --format compact" \
    "\"$TP_BIN\" task list --workspace \"$WORKSPACE\" --status open --ready-only --format compact"
run "tplane link xanadu --workspace \"$WORKSPACE\" --actor agent-a --from \"$task_b_ref\" --to \"$note_ref\" | jq '{relation: .data.relation, transclusion_id: .data.transclusion_id}'" \
...
run "tplane note update --workspace \"$WORKSPACE\" --actor agent-a --note-id \"$note_id\" ... | jq '{title: .data.title, transclusion_id: .data.transclusion_id}'" \
...
run "tplane task show --task-id \"$task_b_id\" | jq '{title: .data.title, priority: .data.priority, owner: .data.owner, labels: .data.labels, transclusion_id: .data.transclusion_id}'" \
